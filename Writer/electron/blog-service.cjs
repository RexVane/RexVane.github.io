...
class BlogService {
  constructor({
    ...
    // owner/repository 从博客仓库的 origin 自动解析（assertRepository），不写死任何账号。
    this.owner = owner;
  ...
  async assertRepository() {
    ...
    const remote = await this.git(['remote', 'get-url', 'origin'], { allowFailure: !this.strictRemote });
    const match = String(remote).trim().match(/github\.com[/:]([^/]+)\/(.+?)(?:\.git)?\/?$/i);
    if (match) {
      [, this.owner, this.repository] = match;
    } else if (this.strictRemote) {
      throw new WriterError('博客仓库的 origin 不是 GitHub 地址，无法发布', 'INVALID_REMOTE');
    }
  }
  ...
  async getRepositoryStatus() {
    ...
    return {
      ...
      remote: this.owner && this.repository ? `${this.owner}/${this.repository}` : '',
    };
  }
  ...
  publishResult(branch, commit, repository) {
    return {
      branch,
      commit,
      compareUrl: this.owner && this.repository
        ? `https://github.com/${this.owner}/${this.repository}/compare/main...${encodeURIComponent(branch)}?expand=1`
        : '',
      repository,
    };
  }
