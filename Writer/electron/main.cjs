...
// 博客工作副本的定位顺序：环境变量 WRITER_REPO_PATH → 配置文件 writer-config.json 的
// repoPath → 自动探测用户主目录下的 *.github.io 文件夹（须含 .git）。
function resolveRepoPath() {
  if (process.env.WRITER_REPO_PATH) return process.env.WRITER_REPO_PATH;
  try {
    const configPath = path.join(app.getPath('userData'), 'writer-config.json');
    const config = JSON.parse(fsSync.readFileSync(configPath, 'utf8'));
    if (config.repoPath) return String(config.repoPath);
  } catch {
    // 没有配置文件时走自动探测。
  }
  const home = os.homedir();
  const candidates = fsSync.readdirSync(home, { withFileTypes: true })
    .filter((entry) => entry.isDirectory() && /\.github\.io$/i.test(entry.name))
    .map((entry) => path.join(home, entry.name))
    .filter((directory) => fsSync.existsSync(path.join(directory, '.git')));
  return candidates[0] || path.join(home, 'blog');
}
...
app.whenReady().then(async () => {
  ...
  handle('writer:open-github', async (payload) => {
    ...
    if (!url.startsWith('https://github.com/')) {
      throw new WriterError('只能打开 GitHub 页面', 'INVALID_URL');
    }
    await shell.openExternal(url);
    return true;
  });
