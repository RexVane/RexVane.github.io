...
function writeAutosave() {
  localStorage.setItem('rexvane-writer-autosave', JSON.stringify({ form: getFormData(), savedAt: Date.now() }));
}

function clearAutosave() {
  clearTimeout(state.autosaveTimer);
  localStorage.removeItem('rexvane-writer-autosave');
}
...
function restoreAutosave() {
  try {
    const saved = JSON.parse(localStorage.getItem('rexvane-writer-autosave') || 'null');
